...
class DUMA(nn.Module):
    def __init__(self, num_scales, in_channels, out_channels, base_A, dropout=0, act_cfg=dict(type='ReLU')):
        ...
        self.base_A = torch.Tensor(base_A)  # Static adjacency (K, V, V)
        self.register_buffer('static_A', self.base_A)
        ...
        # Edge weight generator
        self.edge_gen = nn.Sequential(
            nn.Conv2d(in_channels, num_scales, 1),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, x):
        N, C, T, V = x.shape
        encoded = self.node_encoder(x)  # (N, C/2, T, V)
        
        # Compute dynamic adjacency
        edge_weights = self.edge_gen(x)  # (N, K, T, V)
        dynamic_A = self.static_A.unsqueeze(0) * edge_weights.unsqueeze(-1)  # (N, K, T, V, V)
        
        # Multi-scale aggregation
        support = torch.einsum('nktsv,nctv->nkctv', dynamic_A, encoded)
        support = support.reshape(N, self.num_scales * (C//2), T, V)
        return self.mlp(support)
